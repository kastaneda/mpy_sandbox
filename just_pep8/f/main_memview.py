...
class App:
    ...
    def handle(self, topic, payload, sender=None):
...
class DebugPrint:
    ...
    async def main(self, app):
        sys.stdout.write(b'Main loop started, press Ctrl-C to stop\n')
        while True:
...
class LocalManager:
    ...
    def handle(self, topic, payload):
        if topic == b'btn':
            self.app.handle(b'led/set', payload)
...
class WirelessMQTT:
    def __init__(self, server, prefix, **kwargs):
        ...
        kwargs[b'keepalive'] = 2
        self.mq = umqtt.simple.MQTTClient(prefix, server, **kwargs)
        self.mq.set_last_will(self.prefix+b'online', b'0', retain=True)
    ...
    async def main(self, app):
        ...
        self.mq.connect()
        self.mq.publish(self.prefix+b'online', b'1', retain=True)
        self.mq.subscribe(self.prefix+b'+/set')
    ...
    def mqtt_callback(self, topic, payload):
        if topic.startswith(self.prefix):
            topic = memoryview(topic)[len(self.prefix):]
            self.app.handle(topic, payload, self)
...
class DebugSpeed:
    async def main(self, app):
        ...
        while True:
            ...
            if delta >= 10000:
                app.handle(b'loops', loop_count//10)
                loop_count = 0
                t0 = t1
...
class DebugMem:
    async def main(self, app):
        while True:
            ...
            gc.collect()
            app.handle(b'mem_free', gc.mem_free())
...
try:
    ...
    app.add(SimpleButton(btn, b'btn', True))
    ...
    app.add(WirelessMQTT(b'192.168.0.82', b'board99'))
